...
__global__ void TiledMatrixMulKernel(float* P, float* M, float* N, int MHeight, int MWidth, int NWidth, int TileHeight, int TileWidth) {
...
// Assuming dimensions are M_height and N_width for the P matrix
void compareRandomSamples(float* cpuResults, float* gpuResults, int M_height, int N_width) {
    int samples = 20; // Number of samples you want to compare
    printf("Comparing %d random samples from the output matrices:\n", samples);

    for (int i = 0; i < samples; i++) {
        int row = rand() % M_height; // Random row index
        int col = rand() % N_width;  // Random column index
        int index = row * N_width + col;

        float cpuVal = cpuResults[index];
        float gpuVal = gpuResults[index];

        if (fabs(cpuVal - gpuVal) > 1e-5) {
            printf("Mismatch at row %d, col %d: CPU %f, GPU %f\n", row, col, cpuVal, gpuVal);
        }
        else {
            printf("Match at row %d, col %d: CPU %f, GPU %f\n", row, col, cpuVal, gpuVal);
        }
    }
}
...
int main() {
    ...
    for (int k = 0; k < numExperiments; k++) {
        ...
        for (int row = 0; row < M_height; row++) {
            for (int col = 0; col < N_width; col++) {
                int index = row * N_width + col;
                if (fabs(h_P_cpu[index] - h_P[index]) > 1e-5) {
                    correct = false;
                    break; // Break the inner loop if a mismatch is found
                }
            }
            if (!correct) {
                break; // Break the outer loop as well if a mismatch is found
            }
        }
    ...
    compareRandomSamples(h_P_cpu, h_P, M_height, N_width);
